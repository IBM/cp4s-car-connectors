...
class DataHandler(BaseDataHandler):
    ...
    def create_source_report_object(self):
        if not (self.source and self.report):
            ...
            self.source = {'_key': context().args.CONNECTION_NAME, 'name': 'world', 'description': 'Reference Asset server'}
            current_timestamp = get_report_time()  # Get the current timestamp
            self.report = {'_key': str(current_timestamp), 'timestamp' : current_timestamp, 'type': 'Reference Asset server', 'description': 'Reference Asset server'}
    ...
    def handle_vulnerabilities(self, obj):
        ...
        for xref in obj['xref_properties']:
            # res[f'property{index}'] = res['xref_properties'].append(filter_out(find_by_id(self.xrefproperties, xref), 'pk'))
            res['xref_properties'][f'property{index}'] = filter_out(find_by_id(self.xrefproperties, xref), 'pk')
            index += 1
